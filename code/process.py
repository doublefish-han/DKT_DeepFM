...
def main(args):
    ...
    # student-wise chronological split (70/15/15)
    def split_user(g):
        n = len(g)
        if n <= 1:
    ...
    deepfm_df = deepfm_df.groupby("user_id", sort=False, group_keys=False).apply(split_user)
    ...
    # save splits
    train_df = deepfm_df[deepfm_df["split"] == "train"].drop(columns=["split"])
    val_df   = deepfm_df[deepfm_df["split"] == "val"].drop(columns=["split"])
    test_df  = deepfm_df[deepfm_df["split"] == "test"].drop(columns=["split"])
    ...
    print(f"✅ Saved splits: train/val/test = {len(train_df):,}/{len(val_df):,}/{len(test_df):,}")
