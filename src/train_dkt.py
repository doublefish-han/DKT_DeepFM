...
class KTDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        interactions = skills * 2 + correct.long() + 1  # +1 保留 0 作为 padding
...
class DKT(nn.Module):
    ...
    def forward(self, interaction_ids):
        ...
        return logits, h                    # 同时返回隐藏状态 h_t

def train():
    ds = KTDataset(DATA)
    n_skill = 1 + max(max(d["skills"]) for d in ds.data)
    n_interaction = 2 * n_skill + 1  # 包含 padding_idx=0
    model = DKT(n_interaction).to(DEVICE)
    opt = torch.optim.Adam(model.parameters(), lr=1e-3)
    bce = nn.BCEWithLogitsLoss(reduction='none')  # 用掩码忽略padding
