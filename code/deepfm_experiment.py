...
class ExperimentConfig:
    ...
    mastery_variant: Literal["auto", "none", "full", "zero", "shuffle", "leaky"] = "auto"
    mastery_location: Literal["both", "fm_only", "dnn_only"] = "both"
    max_epochs: int = 30
    patience: int = 5
    batch_size: int = 2048
    val_ratio: float = 0.1
    ...
    def uses_mastery(self) -> bool:
        return self.mastery_variant in {"full", "zero", "shuffle", "leaky"}

    @property
    def run_tag(self) -> str:
        base = self.model_name if self.mastery_variant in {"none", "full"} else f"{self.model_name}_{self.mastery_variant}"
        if self.model_name == "deepfm_dkt" and self.mastery_location != "both":
            return f"{base}_{self.mastery_location}"
        return base
    ...
    def display_name(self) -> str:
        mapping = {
            ...
            ("deepfm_dkt", "leaky"): "Leaky-h_t (audit)",
        }
        name = mapping.get((self.model_name, self.mastery_variant), self.run_tag)
        if self.model_name == "deepfm_dkt" and self.mastery_location != "both":
            suffix = "FM-only" if self.mastery_location == "fm_only" else "DNN-only"
...
def load_dataset(config: ExperimentConfig, seed: int) -> DatasetBundle:
    if config.uses_mastery:
        if config.mastery_variant == "leaky":
            train_path = DATA_DIR / "train_with_mastery_leaky.csv"
            test_path = DATA_DIR / "test_with_mastery_leaky.csv"
        else:
    ...
    val_path = DATA_DIR / "val_with_mastery.csv" if config.uses_mastery else DATA_DIR / "val.csv"
    if not val_path.exists():
        raise FileNotFoundError(f"Expected validation split not found: {val_path}")
    ...
    val_df = val_df.reset_index(drop=True).assign(row_id=lambda df: np.arange(len(df)))
    ...
    if config.uses_mastery:
        ...
        if config.mastery_variant == "zero":
            ...
            if mastery_cols:
                ...
                rng_val = np.random.default_rng(seed + 7)
                for df, rng in ((train_df, rng_train), (val_df, rng_val), (test_df, rng_test)):
                    if len(df):
...
def build_feature_columns(bundle: DatasetBundle, config: ExperimentConfig) -> Tuple[List[SparseFeat], List[DenseFeat], List[str]]:
    ...
    # Split dense columns into mastery vs non-mastery
    mastery_cols = [c for c in bundle.dense_cols if c.startswith("mastery_")]
    non_mastery_cols = [c for c in bundle.dense_cols if not c.startswith("mastery_")]

    # Non-mastery dense features go to both FM and DNN
    for col in non_mastery_cols:
        linear_cols.append(DenseFeat(col, 1))
        dnn_cols.append(DenseFeat(col, 1))

    # Mastery dense features: route depending on config.mastery_location
    if config.mastery_location in ("both", "fm_only"):
        for col in mastery_cols:
            linear_cols.append(DenseFeat(col, 1))
    if config.mastery_location in ("both", "dnn_only"):
        for col in mastery_cols:
            dnn_cols.append(DenseFeat(col, 1))
...
def train_single_seed(config: ExperimentConfig, seed: int) -> SeedResult:
    ...
    linear_cols, dnn_cols, feature_names = build_feature_columns(bundle, config)
    ...
    # Choose threshold on validation set (F1-optimal)
    thresholds = np.linspace(0.0, 1.0, 101)
    f1s = [f1_score(val_true, (val_pred >= t).astype(int)) for t in thresholds]
    ...
    metrics = {
        ...
        "F1": float(f1_score(test_true, (test_pred >= best_t).astype(int))),
...
def compute_ensemble_mean(merged_predictions: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if pred_cols:
        # Force numeric conversion; bad values become NaN so we can drop them.
        probs_df = merged_predictions[pred_cols].apply(pd.to_numeric, errors="coerce")
        probs = probs_df.mean(axis=1, skipna=True)
    elif "prediction" in merged_predictions.columns:
        # Already averaged predictions (e.g., loaded from existing artifacts)
        probs = pd.to_numeric(merged_predictions["prediction"], errors="coerce")
    else:
        # Fallback: try any other numeric value columns (excluding id/label/seed)
        candidate_cols = [
            c for c in merged_predictions.columns if c not in {"row_id", "label", "seed"}
        ]
        if candidate_cols:
            probs_df = merged_predictions[candidate_cols].apply(pd.to_numeric, errors="coerce")
            # Keep only columns that have at least one finite value
            keep_cols = [c for c in probs_df.columns if np.isfinite(probs_df[c].to_numpy(dtype=float)).any()]
            if keep_cols:
                probs = probs_df[keep_cols].mean(axis=1, skipna=True)
            else:
                raise ValueError(
                    "No pred_seed_* or prediction column found, and no numeric columns with finite values. "
                    f"Available columns: {list(merged_predictions.columns)}"
                )
        else:
            raise ValueError(
                "No pred_seed_* or prediction column found; "
                f"available columns: {list(merged_predictions.columns)}"
            )

    out = merged_predictions[["row_id", "label"]].copy()
    out["prob"] = probs
    out = out.loc[np.isfinite(out["prob"].to_numpy(dtype=float))].reset_index(drop=True)
    if out.empty:
        raise ValueError("No finite probabilities available after cleaning.")

    row_ids = out["row_id"].to_numpy(dtype=int)
    labels = out["label"].to_numpy(dtype=int)
    probs_arr = out["prob"].to_numpy(dtype=float)
    return row_ids, labels, probs_arr
...
def plot_roc_pr_curves(
    ...
    finite = np.isfinite(probs_b) & np.isfinite(probs_f)
    labels_b, probs_b = labels_b[finite], probs_b[finite]
    labels_f, probs_f = labels_f[finite], probs_f[finite]
    if len(labels_b) == 0:
        raise ValueError("No finite probabilities available to plot ROC/PR curves.")
...
def paired_bootstrap_from_predictions(
    baseline_preds: pd.DataFrame,
    variant_preds: pd.DataFrame,
    *,
    metric: Literal["AUC", "PR_AUC", "LogLoss", "F1"],
    n_bootstrap: int,
    random_state: int,
) -> Dict[str, float]:
    """Paired bootstrap on per-sample predictions (preferred over bootstrapping over seeds)."""
    # Compute ensemble-mean predictions for each model, then align strictly by (row_id, label)
    row_b, y_true_b, y_prob_b = compute_ensemble_mean(baseline_preds)
    row_v, y_true_v, y_prob_v = compute_ensemble_mean(variant_preds)

    df_b = pd.DataFrame({"row_id": row_b, "label_b": y_true_b, "prob_b": y_prob_b})
    df_v = pd.DataFrame({"row_id": row_v, "label_v": y_true_v, "prob_v": y_prob_v})

    
    merged = df_b.merge(df_v, on=["row_id"], how="inner")
    if merged.empty:
        raise ValueError("No overlapping row_id between baseline and variant predictions.")

    
    mismatch = merged["label_b"].to_numpy() != merged["label_v"].to_numpy()
    if np.any(mismatch):
        bad = merged.loc[mismatch, ["row_id", "label_b", "label_v"]].head(5)
        raise ValueError(
            "Label mismatch after aligning by row_id. This indicates misaligned prediction sets.\n"
            f"Examples:\n{bad.to_string(index=False)}"
        )

    
    mask = np.isfinite(merged["prob_b"].to_numpy(dtype=float)) & np.isfinite(merged["prob_v"].to_numpy(dtype=float))
    merged = merged.loc[mask].reset_index(drop=True)

    if merged.empty:
        raise ValueError(
            "No valid aligned samples for paired bootstrap (after row_id join + finite filtering). "
            "This usually means one model produced NaN/inf probabilities for (almost) all samples."
        )

    y_true = merged["label_b"].to_numpy(dtype=int)
    y_prob_b = merged["prob_b"].to_numpy(dtype=float)
    y_prob_v = merged["prob_v"].to_numpy(dtype=float)
    rng = np.random.default_rng(random_state)
    n = len(y_true)
    if n == 0:
        raise ValueError("Empty prediction set for paired bootstrap.")

    def _score(y, p) -> float:
        """Score helper that is robust to NaN/inf and degenerate bootstrap samples."""
        y = np.asarray(y)
        ...
        try:
            if metric == "AUC":
                # roc_auc_score fails if only one class is present in the sample
                if np.unique(y).size < 2:
                    return float("nan")
                return float(roc_auc_score(y, p))
            if metric == "PR_AUC":
                # average_precision_score is defined for one-class, but keep symmetry with AUC
                if np.unique(y).size < 2:
                    return float("nan")
                return float(average_precision_score(y, p))
            if metric == "LogLoss":
                return float(log_loss(y, clip_probs(p)))
            if metric == "F1":
                return float(f1_score(y, (p >= 0.5).astype(int)))
        except Exception:
            return float("nan")

        raise ValueError(f"Unsupported metric: {metric}")

    # Observed difference (variant - baseline) for higher-is-better metrics;
    # for LogLoss, lower is better, so we flip the sign to keep positive = improvement.
    observed_base = _score(y_true, y_prob_b)
    observed_var = _score(y_true, y_prob_v)
    if metric == "LogLoss":
        observed = observed_base - observed_var
    else:
        observed = observed_var - observed_base
    ...
    for i in range(n_bootstrap):
        ...
        pb = y_prob_b[sample_idx]
        pv = y_prob_v[sample_idx]
        sb = _score(y_s, pb)
        sv = _score(y_s, pv)
        boot[i] = (sb - sv) if metric == "LogLoss" else (sv - sb)

    boot = boot[np.isfinite(boot)]
    if boot.size == 0:
        raise ValueError(
            "Paired bootstrap produced no valid samples (all NaN). "
            "This can happen if probabilities contain NaN/inf or bootstrap samples are degenerate."
        )

    ci_low, ci_high = np.percentile(boot, [2.5, 97.5])
    # Two-sided p-value around 0 improvement
    p_value = float(2.0 * min(np.mean(boot <= 0), np.mean(boot >= 0)))
